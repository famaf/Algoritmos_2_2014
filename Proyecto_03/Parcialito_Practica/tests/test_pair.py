...
class PairTestCase(TestCase):
    ...
    def test_fst(self):
        ...
        self.assertEqual(index_to_string(pair_fst(p)), i)
    ...
    def test_copy_pair_and_destoy_copy_and_operate_on_original(self):
        p = self.new_pair('foo', 'bar')
        other = pair_copy(p)
        pair_destroy(other)

        self.assertEqual(index_to_string(pair_fst(p)), 'foo')
        self.assertEqual(data_to_string(pair_snd(p)), 'bar')
