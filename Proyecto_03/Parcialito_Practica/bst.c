...
data_t bst_search(bst_t arbol, index_t index) {
    ...
    if (arbol != NULL) {
        if (index_is_less_than(index, pair_fst(arbol->elem)))   //index < indice del arbol
        {
            resultado = bst_search(arbol->izquierda, index);
        } else if (index_is_equal(index, pair_fst(arbol->elem)))        //index = indice del arbol
        {
            resultado = pair_snd(arbol->elem);
        } else                  //index > indice del arbol
        {
...
unsigned int bst_leaf_count(bst_t bst) {
    unsigned int hojas = 0;

    if (bst != NULL) {
        if (bst->derecha == NULL && bst->izquierda == NULL) {
            hojas = hojas + 1;
        } else {

            hojas =
                bst_leaf_count(bst->izquierda) + bst_leaf_count(bst->derecha);
        }
    }
